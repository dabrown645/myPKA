...
- **tasks/** - cross-session work continuity. Work that outlives a session becomes a task file under `Team Knowledge/tasks/`, moving through `open/` → `in-progress/` → `done/YYYY/MM/` (or `cancelled/YYYY/MM/`); the running summary view is `tasks/INDEX.md`. Lifecycle procedures are the un-numbered framework SOPs in [[Team Knowledge/SOPs/INDEX|SOPs]]; resource-linking rules live in [[GL-004-task-resource-linking]]. (Task links are basename-only by rule - see [[SOP-rebuild-task-index]] - because task files move between status folders.)
- **[[Team Knowledge/Templates/INDEX|Templates]]** - copy-and-edit starter files for the eight PKM entity types. Frontmatter per [[GL-002-frontmatter-conventions]].
- **[[Team Knowledge/scripts/README|scripts]]** - utility scripts that ship with the scaffold: the one-shot inline-fields migration helper and the three Weekly Reports production scripts.
