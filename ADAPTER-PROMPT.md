...
4. **Personalize the scaffold (one-time, on first activation only).** The scaffold ships with `{{USER_NAME}}` placeholders in files where the prose names the user as the actor. **Replacement scope — read this before touching anything.** Exactly four files DOCUMENT the token mechanism itself and keep their `{{USER_NAME}}` mentions forever: `ADAPTER-PROMPT.md` (this file), the root `AGENTS.md` ("Personalization" section), `CHANGELOG.md` (release history), and the tool-pointer file (`CLAUDE.md` / `GEMINI.md` / equivalent). Mentions inside those four are instructions about the token, not placeholders — never substitute there. Every occurrence in every OTHER file is a genuine placeholder and gets replaced.
   ...
   - **Substitute the TOKEN, never a name.** Replace `{{USER_NAME}}` and nothing else. Do NOT search for a person's name and swap it for the user's, in any file, for any reason — not to make prose read naturally, not because a comment "should" say the user's name, not because another instruction says to address the user by name (that rule governs how you SPEAK to them, never how you EDIT files). A name already sitting in a `.ts`, `.tsx`, `.js`, `.py`, `.css`, `.sql`, `.json` or `.yml` file is authorship or change history. Rewriting it re-attributes someone else's decisions to the user and, worse, silently converts *someone else's* content into what looks like *theirs*. This has happened: a health dashboard once showed one person's overdue referral to a different person under their own name, and the substitution had removed the only clue.
   - **Never edit source code during personalization at all.** Step 4 touches `.md`, `.yaml`, `.yml` and `.txt`. If you believe a source file needs a name change, the answer is that it needs the name REMOVED upstream, not replaced locally — report it instead of fixing it.
